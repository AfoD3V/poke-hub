...
- **Drizzle camelCase column refs in `drizzleSql` — `table.cardId` not `table.card_id`:** Drizzle column objects use camelCase JS property names even when the DB column is snake_case. Using `binderSlots.card_id` in a `drizzleSql` template literal inserts `undefined` and causes a silent query error. Always use the camelCase name (`binderSlots.cardId`, `binderSlots.slotIndex`).
- **`react-hooks` v5 canary — new `set-state-in-effect` and `error-boundaries` rules:** The installed `eslint-plugin-react-hooks@5.0.0-canary` adds two rules not present in v4. `set-state-in-effect` bans calling `setState` (or functions that call it) directly in an `useEffect` body — move synchronous clears to event handlers and inline async fetches into a local `async function` with a `cancelled` flag. `error-boundaries` bans returning JSX from inside a `try` block — collect data in the try, render JSX after.
- **`playwright-cli goto` vs `open` for navigation:** `playwright-cli open <url>` resets the browser session (new context, all cookies lost). After logging in, navigate with `playwright-cli goto <url>` to preserve the auth cookie. Only call `open` once per session.
- **`playwright-cli fill` with `!` — wrap in `printf`:** Shell history expansion turns `!` into a literal `\!` inside double-quoted strings. Pass passwords as `"$(printf '%s' 'Claude123!')"` to prevent expansion.
