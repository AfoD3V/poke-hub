...
The following components SHALL exist as React functional components in `ui-react/src/lib/components/`, each with a co-located `ComponentName.module.css` file containing the verbatim CSS from the corresponding Svelte `<style>` block:
...
- **WHEN** ESLint is run in `ui-react/`
