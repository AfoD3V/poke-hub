...
`ui/src/lib/hooks/useSpring.test.ts` SHALL contain unit tests covering: settle-to-target, stiffness comparison, damping overshoot, and soft-set behaviour. Tests SHALL use `@testing-library/react-hooks` or `renderHook` from `@testing-library/react`.
