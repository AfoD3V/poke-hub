...
The default `--card-glow` CSS variable in `Card.tsx` and `CardModal.tsx` SHALL be `hsl(0, 90%, 44%)` (Pokéball Red) instead of the previous `hsl(215, 90%, 70%)` (blue). All type-specific glow overrides (`.card.water`, `.card.fire`, `.card.grass`, etc.) SHALL remain unchanged and SHALL continue to take precedence over the default.
