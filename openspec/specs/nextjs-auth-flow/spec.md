...
A Server Action `logout()` SHALL exist in `ui/src/app/auth/logout/actions.ts` (or in the Sidebar component). It SHALL call `cookies().delete('pokehub_session')` and redirect to `/auth/login`.
